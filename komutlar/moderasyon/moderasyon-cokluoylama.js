    ...
    try {
        ...
        message.channel.send(pollEmbed).then(async pollMsg => {
            for (let lauraaaa = 0; lauraaaa < lauraaa; lauraaaa++) {
                ...
                pollEmbed.setFooter( "UMEF-EK / Discord'da Yeni Devrim!", client.user.avatarURL())
                pollEmbed.setColor("BLUE")
                pollEmbed.addField(`${emojies[lauraaaa]} ${lauraa[lauraaaa].trim()}`, `\u200B`, true);
            };
            
            await pollMsg.edit(pollEmbed.setTitle(youthanasia).setFooter( "UMEF-EK / Discord'da Yeni Devrim!", client.user.avatarURL()));
        });
    } catch(err) {
